...
%
% There are five (10) exercises in this lab that are to be handed in at the
...
% online using Blackboard. If you have trouble with Blackboard, hand in 
% printouts of both your source code (the m-file) and the html generated
...
%calculating values of x using intervals of h
for j = 1:T
    x(j) = -3 + h*(j-1);
end

%calculating y using values of x
for i = 1:T
    y(i) = abs(x(i))*x(i);
end
...
%y(t) = (t+C)*e^(-t^2) general solution

%initializing t
t = linspace(1, 5);

y1 = t .* exp(-(t.^2)) + (-1 - exp(1)).*(exp(-(t.^2))); %y(1) = -1
y2 = t .* exp(-(t.^2)) + -1 .* (exp(-(t.^2)));          %y(1) = 0
y3 = t .* exp(-(t.^2)) + (exp(1) - 1).*(exp(-(t.^2)));  %y(1) = 1
...
xlabel('t');
ylabel('y(t)');
title('Plotting solution for Differential Equation');
...
%initializing inline function
f = @(x) (x.^3 - abs(x)).*log(x.^2 + 1);

%calculations of function at 0, 1, and -1
f(0)
f(1)
f(-1)
...
%computation of f at various values
f(0, 0)
f(pi/2, 1)
f(-1, -1)
...
%initializing inline function and x, y values
f = @(x, y) y + y.^3 - cos(x) + x.^2 - x.^4 + 1;
xvals = linspace(-2, 2);
yvals = zeros(size(xvals));

%calculation of values of y using fzero function
for j = 1:length(xvals)
    yvals(j) = fzero(@(y) f(xvals(j), y), 100);
...
%0 = y - Ae^(-(sin(x)) + x^3); general solution
%0 = y - exp(-(sin(x)) + x^3); particular solution, where A = 1 when y(0) = 1

%initialization of function and x,y values
f = @(x, y) y - exp(-(sin(x)) + x.^3);
xvals = linspace(-1.5, 1.25);
yvals = zeros(size(xvals));

%calculating values of y using fzero function
for j = 1:length(xvals)
    yvals(j) = fzero(@(y) f(xvals(j), y), 2);
end
...
%% Exercise 11 (NOT TO BE HANDED IN)
%
...
% Label your axes.
%
%% Exercise 12 (NOT TO BE HANDED IN)
%
