...
%
% There are five (8) exercises in this lab that are to be handed in at the
...
% plot.  Write in the comments where the error is largest, and give a
% brief (1-2 sentences) explanation of why it is largest there.  Make sure
% to label your axes.
...
% Error is largest at t = pi/2. The reason this is the case is
% because at t = pi/2, the graph shows the steepest curve or largest dy/dt.
% As a result, the matlab estimator will result in higher inaccuracy near
% those points. 
...
%matlab generates an error because as the solution approaches t = 1, both
%y' and y approach infinity.
...
for i = 1:N
    ...
    newphi = int(func, s, 0 ,t) + sym(1);   % integrate to find next approximation
    
    phi = newphi;      %change old phi to new phi
end

%plotting answer
picard=ezplot(phi,[0,5]);
set(picard,'Color','green'); 

hold on;

%exact solution is y = tan(pi/4 + t)

%plotting exact solution
exact=ezplot(tan(sym(pi/4) + t),[0,5]);
